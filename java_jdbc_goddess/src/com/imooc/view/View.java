...
public class View {
	...
	public static void main(String[] args) throws Exception {
		...
		String previous = null;//标记
		Integer step = 1;//步骤
		
		while(scan.hasNext()){//有输入值就循环，没输入值就不循环
			...
			if(OPERATION_EXIT.equals(in.toUpperCase()) || OPERATION_EXIT.substring(0, 1).equals(in.toUpperCase())){
				...
				List<Goddess> gl =  action.query();
				System.err.println("获取的女神信息为：");
				for(Goddess g: gl){
				...
				if (OPERATION_GET.equals(previous))
				{
					...
					if(goddess != null){
						System.out.println(goddess.getId()+" "+goddess.getUser_name()+" "+
								goddess.getAge()+" "+goddess.getEmail()+" "+goddess.getMobile());
					}
					previous = null;
				}
